    ...
    <div className="min-h-screen relative flex flex-col items-center justify-center overflow-hidden"
      ...
      <div className="relative z-10 flex flex-col items-center" style={{ width: '100%', maxWidth: 520 }}>
        ...
        <motion.div initial={{ opacity: 0, y: -15 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-6">
          <div className="flex items-center justify-center mb-4">
            <div className="relative rounded-full flex items-center justify-center"
              style={{ width: 64, height: 64, background: 'radial-gradient(circle, rgba(235,28,40,0.05) 0%, #F8FAFC 70%)', border: '1px solid rgba(235,28,40,0.15)', boxShadow: '0 2px 12px rgba(0,0,0,0.06)' }}>
              <img src={LOGO_SRC} alt="HWLL" style={{ width: 56, height: 56, objectFit: 'contain' }} />
            </div>
          </div>
          <h2 className="font-title text-xl font-bold tracking-wider" style={{ color: '#16A34A', letterSpacing: '0.12em' }}>
            通用健康套餐
          </h2>
        ...
        <motion.div
          ...
          style={{ background: '#FFFFFF', border: '1px solid rgba(0,0,0,0.08)', boxShadow: '0 1px 8px rgba(0,0,0,0.06)' }}
        >
          {/* Patient info section */}
          <div className="px-6 py-5" style={{ borderBottom: '1px solid rgba(0,0,0,0.06)' }}>
            <p className="font-data text-xs tracking-widest mb-4" style={{ color: '#16A34A', letterSpacing: '0.15em' }}>
              患者基本信息
            </p>
            <div className="grid grid-cols-2 gap-3 mb-3">
              <div>
                ...
                <input value={name} onChange={e => setName(e.target.value)} placeholder="选填" style={inputStyle}
                  onFocus={e => e.target.style.borderColor = 'rgba(22,163,74,0.4)'}
              ...
              <div className="flex gap-2">
                {[{ v: 'male', l: '男' }, { v: 'female', l: '女' }].map(g => (
                  <button key={g.v} onClick={() => setGender(g.v)}
                    ...
                    style={{
                      background: gender === g.v ? 'rgba(22,163,74,0.08)' : '#F8FAFC',
                      border: `1px solid ${gender === g.v ? 'rgba(22,163,74,0.4)' : 'rgba(0,0,0,0.08)'}`,
                      color: gender === g.v ? '#16A34A' : '#64748B',
                    }}>{g.l}</button>
                ))}
              </div>
            </div>
          ...
          <div className="px-6 py-5">
            <p className="font-data text-xs tracking-widest mb-1" style={{ color: '#0D9488', letterSpacing: '0.15em' }}>
              常规血脂数据（选填）
            </p>
            <p className="font-body text-xs mb-4" style={{ color: '#94A3B8' }}>
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.3 }}
          ...
          <button onClick={onBack}
            ...
            style={{ color: '#64748B', border: '1px solid rgba(0,0,0,0.08)', background: '#F8FAFC' }}>
            ← 返回
          </button>
          <button onClick={handleSubmit} disabled={generating}
            ...
            style={{
              background: generating ? 'rgba(22,163,74,0.06)' : 'linear-gradient(135deg, rgba(22,163,74,0.12), rgba(13,148,136,0.10))',
              border: `1px solid ${generating ? 'rgba(22,163,74,0.15)' : 'rgba(22,163,74,0.35)'}`,
              color: generating ? '#94A3B8' : '#16A34A',
              boxShadow: generating ? 'none' : '0 1px 6px rgba(22,163,74,0.1)',
            }}>
        ...
        <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.4 }}
          className="font-body text-xs text-center mt-4" style={{ color: '#94A3B8', lineHeight: 1.6 }}>
